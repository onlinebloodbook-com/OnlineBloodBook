...
<div id="register">
  ...
  <s:form action="loginUpdateSuccessLink1" method="post" enctype="multipart/form-data" theme="css_xhtml" id="formID" cssClass="formular">  
 ...
 <tr>
   ...
   <td> <s:textfield name="regName" value="%{bean.regName}" cssClass="input validate[required,custom[onlyLetter]]" placeholder="Enter Full Name" /> </td>
   ...
   <td> <s:select name="regBloodGroup" id="regBloodGroup" value="%{bean.regBloodGroup}" headerKey="" headerValue="Select Blood Group" list="{'A+','A-','B+','B-','AB+','AB-','O+','O-','other'}" cssClass="inputt validate[required]"/> </td>    
   <td> <label>Date of Birth <span style="color: maroon;">*</span>:</label></td>
   <td> <s:textfield name="regDOB" value="%{bean.regDOB}" cssClass="input validate[required] hajanDatePicker"  /> </td>
  </tr>
  
  <tr>
   <td> <label>Age <span style="color: maroon;">*</span>:</label></td>
   <td> <s:textfield name="regAge" value="%{bean.regAge}" cssClass="input validate[required,custom[onlyNumber]" maxlength="2"/></td>
   <td> <label> weight <span style="color: maroon;">*</span>:</label></td>
   <td> <s:textfield name="regWeight" value="%{bean.regWeight}" cssClass="input validate[required,custom[onlyNumber]" maxlength=""/></td>
  </tr>
  
  <tr>
   <td> <label>Availability <span style="color: maroon;">*</span>:</label></td>
   <td> <s:select headerKey="" headerValue="Select Availability" list="{'available','unavailable'}" name="regBloodAvailability" id="regBloodAvailability" value="%{bean.regBloodAvailability}" cssClass="inputt validate[required]" /> </td>
   <td> <label> Mobile No <span style="color: maroon;">*</span>:</label></td>
   <td> <s:textfield name="regMobileNo" value="%{bean.regMobileNo}" cssClass="input validate[required,custom[telephone]]" placeholder="Enter Mobile Number"/> </td>
  </tr>
  
  <tr>
   <td> <label> Land Line No:</label></td>
   <td> <s:textfield name="regLandNo" value="%{bean.regLandNo}" cssClass="input" /> </td>   
   <td> <label> Email <span style="color: maroon;">*</span>:</label></td>
   <td> <s:textfield name="regEmail" value="%{bean.regEmail}" placeholder="Enter Email Address" cssClass="input validate[required,custom[email]]" /> </td>
  </tr>
  ...
  <tr>
   ...
   <td> <s:textarea name="regAddress" value="%{bean.regAddress}" cssClass="input" /> </td>
  </tr>
